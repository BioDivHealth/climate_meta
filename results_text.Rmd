...
Spearman's rank correlation analysis detected a weak negative correlation between 5-Year Journal Impact Factors and Effect Sizes (Spearman's 𝜌 = `r round(correlation1$estimate, 2)[[1]]`, p = `r round(correlation1$p.value,4)`, Fig. S2A) but no significant correlation between Impact Factors and reported p-value (Spearman's 𝜌 = `r round(correlation2$estimate, 2)[[1]]`, p = `r round(correlation2$p.value,3)`, Fig. S2B).

There was limited evidence of publication bias inferred from funnel plots made by plotting standard errors against respective effect sizes (Fig. S3). Egger's regression tests for funnel plot asymmetry showed significant asymmetry of funnel plots for the overall dataset (p = `r round(list_regtest$All,3)`) and precipitation data (P-value = `r round(list_regtest$Precipitation,3)`), but no asymmetry was identified within humidity and temperature data (Fig. S3, Table S7).
