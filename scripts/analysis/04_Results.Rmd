...
old = read.csv(here::here("data","dataset_final.csv"))
df = unique(df) #ensure all rows are unique

# Keep new records:
df = df %>% filter(!Reference_ID %in% c("118", "164", "214"))
...
# helper that counts unique years covered by a Study_period string
count_years <- function(x) {
  if (is.na(x) || x == "") return(NA_integer_)
  # normalise en/em/minus dashes to hyphen
  x <- str_replace_all(x, "[\u2012-\u2015\u2212]", "-")
  # split on comma/semicolon into segments
  segs <- unlist(strsplit(x, "\\s*[,;]\\s*"))
  yrvec <- lapply(segs, function(s) {
    ys <- str_extract_all(s, "\\d{4}")[[1]] |> as.integer()
    if (length(ys) == 0) return(integer(0))     # no year found
    if (length(ys) == 1) return(ys[1]:ys[1])    # single-year segment
    ys[1]:ys[2]                                  # inclusive range
  }) |> unlist()
  if (length(yrvec) == 0) return(NA_integer_)
  length(unique(yrvec))
}

result <- df %>%
  select(Reference_ID, Study_period) %>%
  distinct() %>%
  mutate(years_included = map_int(Study_period, count_years))

result %<>% filter(!is.na(years_included))

summary(result$years_included)
```
...
df <- df |> left_join(oa_tbl, by = "Journal_std")
View(df %>% select(Journal_std, oa_2yr_mean_citedness, oa_h_index, Journal_5yr_Impact_merged) %>% distinct())
...
print(shapiro.test((dat_pub1$oa_2yr_mean_citedness)))

# Do correlation test for all data points
spearman_corr <- cor.test(dat_pub1$oa_2yr_mean_citedness, 
                          abs(dat_pub1$es), 
                          method = "spearman", 
                          exact = FALSE)
print(spearman_corr)
...
iqr_outliers <- dat_pub1 %>% filter(oa_2yr_mean_citedness < lower_bound |
...
print(unique(iqr_outliers$oa_2yr_mean_citedness))

# Remove outliers
dat_pub1 %<>% filter(!oa_2yr_mean_citedness %in% unique(iqr_outliers$oa_2yr_mean_citedness))
...
dat_pub2 = pub2
dat_pub2$P.value_specific = as.numeric(dat_pub2$P.value_specific)
...
spearman_corr <- cor.test(dat_pub2$P.value_specific, dat_pub2$oa_2yr_mean_citedness, method = "spearman", exact = FALSE)
print(spearman_corr)
...
lower_bound <- Q1 - 1.5 * IQR
upper_bound <- Q3 + 1.5 * IQR
iqr_outliers2 <- dat_pub2 %>% filter(oa_2yr_mean_citedness < lower_bound | oa_2yr_mean_citedness > upper_bound)
...
# Print the outliers detected
cat("Outliers detected by IQR method:\n")
print(unique(iqr_outliers2$oa_2yr_mean_citedness))

dat_pub2 %<>% filter(!oa_2yr_mean_citedness %in% unique(iqr_outliers2$oa_2yr_mean_citedness))

# P.value_specific ! ----|
# Calculate IQR
Q1 <- quantile(dat_pub2$P.value_specific, 0.25, na.rm = TRUE)
Q3 <- quantile(dat_pub2$P.value_specific, 0.75, na.rm = TRUE)
IQR <- Q3 - Q1
...
```{r fig3, echo=FALSE}
source(here::here('scripts','figures','Figure_3_Combined.R'))

print(s3)

# ggsave(s3, file=here('outputs','final_figs_new','Figure3_2.png'), device="png", units="in", width=8, height=6, dpi=600, scale=1.1)
# ggsave(s3, file=here('outputs','final_figs_new','Figure3_2.pdf'), device="pdf", units="in", width=8, height=6, dpi=600, scale=1.1)
```
...
```{r FigS2}
source(here::here('scripts','figures','FigureS2.R'))


FS2 <- (p3 / plot_spacer() /p3_rm) + plot_layout(heights = c(1,0.05,1))
FS2a <- (p3 / plot_spacer() /p3_both) + plot_layout(heights = c(1,0.05,1))
FS2_b <- (p3 / plot_spacer() /p3_rm / plot_spacer() / p3_both) + plot_layout(heights = c(1,0.05,1, 0.05, 1))


ggsave(FS2a, file=here('outputs','final_figs_new','FigureS2.pdf'), device="pdf", 
...
source(here::here('scripts','figures','FigureS3.R'))
...
# ggsave(combined_plot_s4, file=here('outputs','final_figs_new','FigureS4.pdf'), device="pdf", units="in", width=6.3, height=3.8, dpi=1000, scale=1.1)
```
