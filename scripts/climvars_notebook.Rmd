...
perform_stat_tests_subset_bootstrap_single_test <- function(data, effect_cat, agreement_var, bin_var, climate_type,
  ...
  cat(paste("\nUsing", test_type, "for all bootstrapped iterations.\n"))
  ...
  if (test_type == "Chi-Square Test") {
    cat("\nChi-Square Tests performed:", num_iterations, "\n")
  } else {
    cat("\nFisher's Exact Tests performed:", num_iterations, "\n")
  }
...
summarize_bootstrap_results <- function(summary_results) {
  ...
  if (test_type == "Chi-Square Test") {
    ...
    summary_list$mean_test_statistic <- mean_stat
    
    # Calculate 95% CI for test statistics using t.test
    stat_ci <- t.test(valid_test_stats, conf.level = 0.95)$conf.int
    summary_list$test_stat_ci_lower <- stat_ci[1]
    summary_list$test_stat_ci_upper <- stat_ci[2]
  } else {
    # For Fisher's Exact Test (or other tests without test statistics), skip this part
    summary_list$mean_test_statistic <- NA
    summary_list$test_stat_ci_lower <- NA
    summary_list$test_stat_ci_upper <- NA
  }
...
# Create and display a nicely formatted contingency table using the function `create_contingency_table`
# Create and display a nicely formatted contingency table using the function `create_contingency_table`
