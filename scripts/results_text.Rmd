...
```{r}
chi_overall <- read.csv(here('outputs', 'tables', 'chisq_results_overall.csv'))
chi_temp <- read.csv(here('outputs', 'tables', 'chisq_results_temperature.csv'))
chi_prec <- read.csv(here('outputs', 'tables', 'chisq_results_precipitation.csv'))
chi_hum <- read.csv(here('outputs', 'tables', 'chisq_results_humidity.csv'))
```
...
When dropping groups from the subsets examining environmental categories individually, levels of significance of some proportions within temperature and precipitation subsets also changed. Temperature subset showed significantly more increases than decreases in disease risk after removal of Hantaviral diseases (n = `r  sum(tmp_samples$General_Disease == "Hantaviral diseases")`; `r paste0(tmp_samples %>% summarize(percentage = mean(General_Disease == "Hantaviral diseases") * 100) %>% round(1), "%")` of the subset; $\overline{\chi^2}$ = `r round(subset(chi_temp, Group == "Hantaviral.diseases")$Chi_Square,2)`, df=1, p= `r round(subset(chi_temp, Group == "Hantaviral.diseases")$P_value,3)`), rodents as a reservoir (n = `r  sum(tmp_samples$Principal_reservoir == "Rodents")`; `r paste0(tmp_samples %>% summarize(percentage = mean(Principal_reservoir == "Rodents") * 100) %>% round(1), "%")` of the subset; $\overline{\chi^2}$ = `r round(subset(chi_temp, Group == "Rodents")$Chi_Square,2)`, df=1, p= `r round(subset(chi_temp, Group == "Rodents")$P_value,3)`) or when including only vectored diseases (n = `r  sum(tmp_samples$Transmission_type == "Vectored")`; `r paste0(tmp_samples %>% summarize(percentage = mean(Transmission_type == "Vectored") * 100) %>% round(1) %>% pull(), "%")` of the subset; $\overline{\chi^2}$ = `r round(subset(chi_temp, Group == "Non.vectored")$Chi_Square,2)`, df=1, p= `r round(subset(chi_temp, Group == "Non.vectored")$P_value,3)`). Simultaneously, precipitation subset no longer had significantly more increases than decreases in disease risk when rodents as a reservoir (n = `r  sum(tmp_samples$Principal_reservoir == "Rodents")`; `r paste0(prec_samples %>% summarize(percentage = mean(Principal_reservoir == "Rodents") * 100) %>% round(1), "%")` of the subset; $\overline{\chi^2}$ = `r round(subset(chi_prec, Group == "Rodents")$Chi_Square,2)`, df=1, p= `r round(subset(chi_prec, Group == "Rodents")$P_value,3)`) or non-vectored diseases (n = `r  sum(tmp_samples$Transmission_type == "Non-vectored")`; `r paste0(tmp_samples %>% summarize(percentage = mean(Transmission_type == "Non-vectored") * 100) %>% round(1), "%")` of the subset; $\overline{\chi^2}$ = `r round(subset(chi_prec, Group == "Non.vectored")$Chi_Square,2)`, df=1, p= `r round(subset(chi_prec, Group == "Non.vectored")$P_value,3)` ) were removed.
...
Pearson's product-moment correlation analysis showed no significant correlation between 5-Year Journal Impact Factors and Hedge's g or reported p-values, either when using the full dataset (Impact Factors and Hedge's g: r = `r round(correlation1$estimate,2)[[1]]`, p = `r round(correlation1$p.value,2)[[1]]`, df = `r correlation1$parameter[[1]]`; Impact Factors and p-values: r = `r round(correlation2$estimate,2)[[1]]`, p = `r round(correlation2$p.value,2)[[1]]`, df = `r correlation2$parameter[[1]]`), or after removing the outliers among Hedge's g and reported p-values (Impact Factors and Hedge's g: r = `r round(correlation1_rm$estimate,2)[[1]]`, p = `r round(correlation1_rm$p.value,2)[[1]]`, df = `r correlation1_rm$parameter[[1]]`; Impact Factors and p-values: r = `r round(correlation2_rm$estimate,2)[[1]]`, p = `r round(correlation2_rm$p.value,2)[[1]]`, df = `r correlation2_rm$parameter[[1]]`).

There was limited evidence of publication bias inferred from funnel plots made by plotting standard errors against respective effect sizes (Fig. S3). Egger's regression tests for funnel plot asymmetry showed significant asymmetry of funnel plots for the overall dataset (z = `r round(regtest_all$zval,2)`, p = `r round(list_regtest$All,3)`) and precipitation data (z = `r round(regtest_prec$zval,2)`, p = `r round(list_regtest$Precipitation,3)`), but no asymmetry was identified within humidity and temperature data (Fig. S3, Table S7).

Furthermore, p-values extracted from the studies were examined to check for potential evidence of "p-hacking". Visual inspection of the distribution of p-values surrounding p = 0.05 found limited evidence of p-hacking, with a small spike in the number of studies with p-values just under p = 0.05 (Fig. S4).
...
```{r}
source(here('scripts','AD_KS_tests.R'))
# Creates: 
...
results_df %>% filter(p_val_removed < 0.05)
round(es_summaries$mn[es_summaries$Transmission_type=="Non-vectored" & es_summaries$Environmental_condition=="Precipitation"],2)
round(es_summaries$sd[es_summaries$Transmission_type=="Non-vectored" & es_summaries$Environmental_condition=="Precipitation"],2)
es_summaries$n[es_summaries$Transmission_type=="Non-vectored" & es_summaries$Environmental_condition=="Precipitation"]
```


However, the effect size distributions for vector-borne diseases and diseases vectored by mosquitoes significantly differed from the overall effect size distribution for temperature (Anderson-Darling Test: vector borne disease: AD = `r round(results_df$stat[results_df$Environmental_condition=="Temperature" & results_df$Group=="Vectored"],2)` ; p=`r results_df$p_val[results_df$Environmental_condition=="Temperature" & results_df$Group=="Vectored"]`; mosquito vectored disease: AD = `r round(results_df$stat[results_df$Environmental_condition=="Temperature" & results_df$Group=="Mosquito"],2)` ; p=`r results_df$p_val[results_df$Environmental_condition=="Temperature" & results_df$Group=="Mosquito"]`). Additionally, the effect size distribution for diseases with a livestock principal reservoir differed from the overall humidity effect size distribution (Anderson-Darling Test: AD = `r round(results_df$stat[results_df$Environmental_condition=="Humidity" & results_df$Group=="Livestock"],2)` ; p=`r results_df$p_val[results_df$Environmental_condition=="Humidity" & results_df$Group=="Livestock"]`). When the subsets were compared to the full distributions with the subset removed, a few more categories were significantly different: non-vectored diseases and diseases with a mammal reservoir significantly differed from the overall temperature distribution, and diseases with a livestock principal reservoir differed from the overall precipitation distribution (Supp. Table SX).

Vector-mediated zoonotic diseases had significantly more positive effects (mean=`r round(es_summaries$mn[es_summaries$Transmission_type=="Vectored" & es_summaries$Environmental_condition=="Temperature"],2)`; sd=`r round(es_summaries$sd[es_summaries$Transmission_type=="Vectored" & es_summaries$Environmental_condition=="Temperature"],2)`; n=`r es_summaries$n[es_summaries$Transmission_type=="Vectored" & es_summaries$Environmental_condition=="Temperature"]`) reported for temperature when compared to solely animal-hosted diseases (mean=`r round(es_summaries$mn[es_summaries$Transmission_type=="Non-vectored" & es_summaries$Environmental_condition=="Temperature"],2)`; sd=`r round(es_summaries$sd[es_summaries$Transmission_type=="Non-vectored" & es_summaries$Environmental_condition=="Temperature"],2)`; n=`r es_summaries$n[es_summaries$Transmission_type=="Non-vectored" & es_summaries$Environmental_condition=="Temperature"]`) (Two-sample Kolmogorov-Smirnov test; D=`r round(test_stats_ks$Temperature[[1]],2)`; p = `r round(pvals_ks$Temperature,3)`; Fig. 3). The distribution of effect sizes for precipitation centred around 0 for both vectored (mean=`r round(es_summaries$mn[es_summaries$Transmission_type=="Vectored" & es_summaries$Environmental_condition=="Precipitation"],2)`; sd=`r round(es_summaries$sd[es_summaries$Transmission_type=="Vectored" & es_summaries$Environmental_condition=="Precipitation"],2)`; n=`r es_summaries$n[es_summaries$Transmission_type=="Vectored" & es_summaries$Environmental_condition=="Precipitation"]`) and non-vectored (mean=`r round(es_summaries$mn[es_summaries$Transmission_type=="Non-vectored" & es_summaries$Environmental_condition=="Precipitation"],2)`; sd=`r round(es_summaries$sd[es_summaries$Transmission_type=="Non-vectored" & es_summaries$Environmental_condition=="Precipitation"],2)`; n=`r es_summaries$n[es_summaries$Transmission_type=="Non-vectored" & es_summaries$Environmental_condition=="Precipitation"]`) diseases. There was no significant difference between these distributions (Two-sample Kolmogorov-Smirnov test; D=`r round(test_stats_ks$Precipitation[[1]],2)`; p = `r round(pvals_ks$Precipitation,3)`), but the distribution of effect sizes for precipitation had qualitatively larger tails for host only diseases compared to host-vector diseases (Fig. 3B). Humidity for non-vectored diseases had effects mainly clustered around zero (mean=`r round(es_summaries$mn[es_summaries$Transmission_type=="Non-vectored" & es_summaries$Environmental_condition=="Humidity"],2)`; sd=`r round(es_summaries$sd[es_summaries$Transmission_type=="Non-vectored" & es_summaries$Environmental_condition=="Humidity"],2)`; n=`r es_summaries$n[es_summaries$Transmission_type=="Non-vectored" & es_summaries$Environmental_condition=="Humidity"]`), while vectored diseases tended towards more positive effects (mean=`r round(es_summaries$mn[es_summaries$Transmission_type=="Vectored" & es_summaries$Environmental_condition=="Humidity"],2)`; sd=`r round(es_summaries$sd[es_summaries$Transmission_type=="Vectored" & es_summaries$Environmental_condition=="Humidity"],2)`; n=`r es_summaries$n[es_summaries$Transmission_type=="Vectored" & es_summaries$Environmental_condition=="Humidity"]`), but this difference was not significant (Two-sample Kolmogorov-Smirnov test; D=`r round(test_stats_ks$Humidity[[1]],2)`; p = `r round(pvals_ks$Humidity,3)`).
...
tmp = df %>%
    filter(Direction != "" & P.value_general != "" &
             P.value_general != ">0.05") 
```


We used Pearson's Chi-squared Test for Count Data (through {stats} R package) to investigate if the proportions of reported increases and decreases in disease risk were significantly different. Specifically, we performed Chi-squared tests on 1000 bootstrap samples, each comprising 80% of the data, for the entire dataset and for the three climate subsets: temperature, precipitation, and humidity. To ensure robustness, we repeated the procedure by sequentially removing records from one major group at a time (e.g. separately removing records from each of the top three diseases) from the analysis, to identify which categories significantly affected the proportions of increases to decreases in disease risk. The groups removed in this process and the percentage of the analysed dataset these groups represent included: Hantaviral diseases (`r paste0(tmp %>% summarize(percentage = mean(General_Disease == "Hantaviral diseases") * 100) %>% round(1), "%")`), Arboviral diseases (`r paste0(tmp %>% summarize(percentage = mean(General_Disease == "Arboviral diseases") * 100) %>% round(1), "%")`), Leptospirosis (`r paste0(tmp %>% summarize(percentage = mean(General_Disease == "Leptospirosis") * 100) %>% round(1), "%")`) (top three diseases); China (`r paste0(tmp %>% summarize(percentage = mean(Country == "China") * 100) %>% round(1), "%")`), Iran (`r paste0(tmp %>% summarize(percentage = mean(Country == "Iran") * 100) %>% round(1), "%")`), USA (`r paste0(tmp %>% summarize(percentage = mean(Country == "USA") * 100) %>% round(1), "%")`) (top three countries); rodents (`r paste0(tmp %>% summarize(percentage = mean(Principal_reservoir == "Rodents") * 100) %>% round(1), "%")`), mammals - multispecies (`r paste0(tmp %>% summarize(percentage = mean(Principal_reservoir == "Mammals (multispecies)") * 100) %>% round(1), "%")`), livestock ( `r paste0(tmp %>% summarize(percentage = mean(Principal_reservoir == "Livestock") * 100) %>% round(1), "%")`) (top three principle reservoirs); viruses (`r paste0(tmp %>% summarize(percentage = mean(Pathogen == "Virus") * 100) %>% round(1), "%")`), bacteria (`r paste0(tmp %>% summarize(percentage = mean(Pathogen == "Bacteria") * 100) %>% round(1), "%")`), parasites (`r paste0(tmp %>% summarize(percentage = mean(Pathogen == "Parasite") * 100) %>% round(1), "%")`) (top three pathogen types); Pearson correlation (`r paste0(tmp %>% summarize(percentage = mean(Statistical_method == "Pearson correlation") * 100) %>% round(1), "%")`), Spearman rank correlation (`r paste0(tmp %>% summarize(percentage = mean(Statistical_method == "Spearman rank correlation") * 100) %>% round(1), "%")`), negative binomial regression (`r paste0(tmp %>% summarize(percentage = mean(Statistical_method == "Negative binomial regression") * 100) %>% round(1), "%")`) (top three statistical methods).