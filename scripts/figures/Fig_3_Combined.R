...
build_A1_ridges_transmission <- function(dd) {
  dd %>%
    ...
    geom_density_ridges(scale = 10, linewidth = 0.1) +
...
build_B1_snake_transmission <- function(dd) {
  ann_text <- data.frame(
    es = c(-2.5, -2.5),
    esrank = c(90, -10),
    lab = c("Non-vectored", "Vectored"),
    Transmission_type = c("Non-vectored", "Vectored"),
  ...
  dd %>%
    ...
    geom_vline(xintercept = 0, linetype = 2) +
    geom_errorbarh(aes(xmin = es - se, xmax = es + se, color = Transmission_type), height = 0, show.legend = FALSE, size = 0.20, alpha = 0.7) +
    scale_color_manual(values = cols_transmission, name = "Transmission Type") +
    geom_text(data = ann_text, aes(label = lab), color = "black", hjust = 0, alpha = 0.7) +
    facet_grid(~Environmental_condition) +
    labs(x = "", y = "") +
...
build_B2_snake_disease <- function(dd) {
  custom_order <- c("HFRS", "Brucellosis", "Leptospirosis", "Scrub typhus", "Japanese encephalitis", "Tick-borne encephalitis", "West Nile Virus")
  ann_text <- data.frame(
    es = rep(-2.5, length(custom_order)),
    esrank = c(143, 115, 90, 68, 49, 23, 5),
    lab = custom_order,
  ...
  dd %>%
    ...
    geom_vline(xintercept = 0, linetype = 2) +
    geom_errorbarh(aes(xmin = es - se, xmax = es + se, color = Disease), height = 0, show.legend = FALSE, size = 0.25, alpha = 0.7) +
    scale_color_manual(values = cols_disease) +
    geom_text(data = ann_text, aes(label = lab), color = "black", vjust = -0.5, hjust = 0, alpha = 0.5) +
    facet_grid(~Environmental_condition) +
