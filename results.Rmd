...
tmp %<>% filter(!is.na(P.value_specific)) %>% filter(P.value_specific<=0.06)

pv = as.data.frame(tmp$P.value_specific)
# Histogram with kernel density
ggplot(pv, aes(x = tmp$P.value_specific, y = after_stat(density))) + 
  geom_histogram(colour = 1, fill = "skyblue4", binwidth = 0.005, boundary = 0, alpha = 0.7) +
...
pv <- as.data.frame(tmp$P.value_specific)
colnames(pv) <- "P_value_specific"

# Histogram with kernel density
ggplot(pv, aes(x = P_value_specific, y = ..density..)) + 
  geom_histogram(aes(fill = "Histogram"), colour = "black", binwidth = 0.005, boundary = 0, alpha = 0.7) +
  geom_density(aes(fill = "Density"), alpha = 0.5) +
  geom_vline(xintercept = 0.05, linetype = "dashed", color = "red", aes(color = "Threshold")) +
  labs(x = "p-value", y = "Density", title = "Density Plot of P.value_specific") +
  scale_fill_manual(name = "Legend", values = c("Histogram" = "skyblue4", "Density" = "pink")) +
  scale_color_manual(name = "Legend", values = c("Threshold" = "red")) +
  theme_minimal()
```
