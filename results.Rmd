...
# Unify P-values
tmp$P.value_general[tmp$P.value_general %in% c("<.0.05", "<0..05")] <- "<0.05"

# Filter specific P-values for the plots
tmp1 = tmp %>% filter(!is.na(P.value_specific)) %>% filter(P.value_specific <= 0.07)
...
pl1 = ggplot(pv1, aes(x = P_value_specific, y = after_stat(density))) + 
  geom_histogram(aes(y = after_stat(density)), colour = 1, fill = "#3C5488FF", binwidth = 0.005, boundary = 0, alpha = 0.8) +
  geom_density(fill = "pink", alpha = 0.4) +
  geom_vline(xintercept = 0.05, linetype = "dashed", color = "#E64B35FF") +
  labs(x = "p-value", y = "Density", title = "Density Plot of p-val < 0.07") +
  theme_linedraw()+
...
pl2 = ggplot(pv2, aes(x = P_value_specific, y = after_stat(density))) + 
  geom_histogram(aes(y = after_stat(density)), colour = 1, fill = "#3C5488FF", binwidth = 0.04, boundary = 0, alpha = 0.8) +
  geom_density(fill = "pink", alpha = 0.4) +
  geom_vline(xintercept = 0.05, linetype = "dashed", color = "#E64B35FF") +
  labs(x = "p-value", y = "Density", title = "Density Plot of p-values") +
  theme_minimal() +
  xlim(0, 1)   # Adjust the x-axis limit based on your data range

# Combine the two plots with an inset
...
#ggsave(combined_plot, file="outputs/FigS4.png", device="png", units="in", width=6.3, height=3.8, dpi=1000, scale=0.95)
```
...
pl1 = ggplot(pv1, aes(x = P_value_specific, y = after_stat(count))) + 
  geom_histogram(aes(y = ..ndensity..), colour = 1, fill = "#3C5488FF", binwidth = 0.005, boundary = 0, alpha = 0.8) +
  geom_density(aes(y = ..scaled..),fill = "pink", alpha = 0.4) +
  geom_vline(xintercept = 0.05, linetype = "dashed", color = "#E64B35FF") +
  labs(x = "p-value", y = "Density", title = "Density Plot of p-val < 0.07") +
  theme_linedraw()+
  theme(axis.text.y = element_text(size = 8),  # Bold y-axis text
...
# Create the full range plot (pl2)
pl2 = ggplot(pv2, aes(x = P_value_specific)) + 
  geom_histogram(aes(y = (..ndensity..)), colour = 1, fill = "#3C5488FF", binwidth = 0.04, boundary = 0, alpha = 0.8) +
  geom_density(aes(y = (..scaled..)),fill = "pink", alpha = 0.4) +
  geom_vline(xintercept = 0.05, linetype = "dashed", color = "#E64B35FF") +
  labs(x = "p-value", y = "Density", title = "Density Plot of p-values") +
  theme_minimal() +
...
tmp %<>% dplyr::select(.,se, es, Reference_ID)
