...
## Impact factors vs Effect Size/p-value
...
correlation1 <- cor.test(dat_pub1$es, dat_pub1$Journal_5yr_Impact, method = "pearson")
cat("Impact Factors vs Effect Sizes:",
    "\nPearson r:", round(correlation1$estimate, 2),
...
correlation2 <- cor.test(dat_pub2$Journal_5yr_Impact, dat_pub2$P.value_specific, method = "pearson")
cat("Impact Factors vs reported p-values:",
    "\nPearson r:", round(correlation2$estimate, 2),
...
spearman_corr <- cor.test(dat_pub1$Journal_5yr_Impact, abs(dat_pub1$es), method = "spearman", exact = FALSE)
print(spearman_corr)

# p-VALUE VS IF -----------------------------
# Perform Shapiro-Wilk test for normality
#dat_pub2 %<>% filter(P.value_specific!=0)
...
# Calculate Spearman's rank correlation
spearman_corr <- cor.test(dat_pub2$P.value_specific, dat_pub2$Journal_5yr_Impact, method = "kendall", exact = FALSE)
print(spearman_corr)

# Scatter plot with a trend line for dat_pub1
ggplot(dat_pub1, aes(x = Journal_5yr_Impact, y = es)) +
  geom_point() +
  geom_smooth(method = "loess", se = TRUE, color = "blue") +
  labs(title = "Scatter Plot of es vs Journal_5yr_Impact",
       x = "Journal 5-year Impact",
       y = "es") +
  theme_minimal()

ggplot(dat_pub2, aes(y = P.value_specific, x = Journal_5yr_Impact)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
  labs(title = "Scatter Plot of P.value_specific vs Journal_5yr_Impact",
       x = "P.value_specific",
       y = "Journal 5-year Impact") +
  theme_minimal()
...
ggplot(dat_pub1, aes(x = Journal_5yr_Impact, y = es)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
  labs(title = "Scatter Plot of Journal 5-year Impact vs Effect Size",
       x = "Journal 5-year Impact",
       y = "Effect Size (es)") +
  theme_minimal()
```
...
ggsave(p3, file="outputs/FigureS2_spearman.png", device="png", units="in", width=9.3, height=4, dpi=1000, scale=1)
```
...
ggsave(s3, file="outputs/FigureS3.png", device="png", units="in", width=8.5, height=5, dpi=1000, scale=1)
```
